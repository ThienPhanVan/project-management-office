...
@Component({
  selector: 'app-notification-reply-comment-news',
  templateUrl: './notification-reply-comment-news.component.html',
  styleUrls: ['./notification-reply-comment-news.component.scss'],
})
export class NotificationReplyCommentNewsComponent implements OnInit {
  @Input() data: any = {};
  ...
  constructor(
    ...
    private productDataService: CommerceDataService,
  ...
  setResult(ev: any, data: any) {
    ...
    this.actionSheet.emit({ event, data, action: 'reply_comment_news' });
  }
      ...
      (res: any) => {
        if (res) {
          ...
          switch (this.data.news.type) {
            case 0:
              this.router.navigate([`/tabs/opportunities/${id}?comment_id=${this.data?.comment.id}`]);
              break;
            case 1:
              this.router.navigate([`/tabs/events/${id}?comment_id=${this.data?.comment.id}`]);
              break;
            default:
              this.router.navigate([`/tabs/news/${id}?comment_id=${this.data?.comment.id}`]);
          }
        }
      },
        (error: any) => {
